...
//자바스크립트 함수 선언
function play_w(directw){ //다이렉트는 좌우 슬라이드방향
    if(directw == 'right'){
        wnum = wnum +1; //wnum++
        if(wnum>2){wnum = 0}
...
$(".rollingbtn").find('li.seq a').each(function(){
    //슬라이드 이미지아래의 이동버튼을 초기값으로 동그랑땡으로 만든다.(아래코드)
    $(".rollingbtn li.seq a img").attr('src', $(".rollingbtn li.seq a img").attr('src').replace('_on.png','_off.png'));
});
//".rolingbtn li.butt0 a img" ->" .rollingbtn li.butt1 a img"->" .rollingbtn li.butt2 a img"
$(".rollingbtn li.butt"+wnum+ " a img").attr('src',$(".rollingbtn li.butt"+wnum+ " a img").attr('src').replace('_off.png','_on.png'));
//조건 비교 == 값비교 ,, ===값,형태비교
if(wnum == 0) {
    $(".viewImgList li.imglist1").animate({'opacity' : 0},1000);//슬라이드 인덱스가 0번일 때 li태그를 1초간 서서히 사라지게 한다.
    $(".viewImgList li.imglist2").animate({'opacity' : 0},1000);
    $(".viewImgList li.imglist0").animate({'opacity' : 1},1000);
}else if(wnum == 1){
    $(".viewImgList li.imglist0").animate({'opacity' : 0},1000);//슬라이드 인덱스가 1번일 때 li태그를 1초간 서서히 사라지게 한다.
    $(".viewImgList li.imglist2").animate({'opacity' : 0},1000);
    $(".viewImgList li.imglist1").animate({'opacity' : 1},1000);
}else if(wnum ==2){
    $(".viewImgList li.imglist0").animate({'opacity' : 0},1000);//슬라이드 인덱스가 2번일 때 li태그를 1초간 서서히 사라지게 한다.
