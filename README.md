...
#### 20201104(목)
-CSS : 계층 구조에서 > 의미는 
    1.$("ul li");  ul안쪽의 모든 하위계층 li태그
    2.$("ul >li");  ul안쪽의 대메뉴만 포함
    <ul>
    <li>대매뉴
            <ul>
                    <li>중매뉴</li>
...
DOM엘리먼트 : <body><p><div><input><label>
-돔을 제어할때 사용 : j 쿼리(자바스크립 라이브러리)-ex)동적으로 <label>태그같은 DOM을 추가 /삭제/  스타일
-$(".roll");  -> 셀렉터(클래스명)
-$("#apple");  -> 셀렉터(아이디명)
-$("div");  -> 셀렉터(태그자체 명)
-$("div, .agree") -> 셀렉터(2개 동시 사용)
...
-var bigData=[
{name:'하나약국',maskcnt:200,lat:300,
    lon:200},
    {name:'둘약국',maskcnt:200,lat:300,
    lon:200},
    {name:'셋약국',maskcnt:200,lat:300,
    lon:200}
];
-alert(bigData[0][0]); ->하나약국

#### 20201104(수)
-input 입력태그 : type[text | password]
                             [ radio | checkbox]
